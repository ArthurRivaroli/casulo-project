...
### Orçamento total = receita do mês; correção de overflow no Safari/iPhone
- **"Orçamento total" agora vem da soma das receitas (`INCOME`) lançadas no mês**, não da soma dos orçamentos definidos por categoria — pedido explícito do usuário (o teto do que dá pra gastar é o que entrou, não um valor arbitrário somado por categoria). [src/app/(dashboard)/orcamentos/page.tsx](<src/app/(dashboard)/orcamentos/page.tsx>): novo `prisma.transaction.aggregate` com `type: "INCOME"` no mês; os orçamentos por categoria continuam existindo e editáveis normalmente, só a métrica do resumo total mudou de fonte
- **Overflow no Safari/iPhone**: usuário reportou caixas "estourando" no celular. Bug raiz identificado (rodei um agente de exploração + confirmei visualmente com Chromium em 375px, já que este ambiente não tem o motor WebKit instalado, só Chromium): linhas `flex items-center justify-between` que colocam um texto de tamanho variável (descrição de transação, nome de conta/categoria/usuário, email) ao lado de um elemento de largura fixa (valor, ícones de editar/excluir, badge) sem `min-w-0` no lado do texto — o padrão flexbox faz o item recusar encolher abaixo do conteúdo, e o Safari é mais estrito que o Chrome nisso, então o valor/ícones acabavam colados ou sobrepostos ao texto em vez de ficarem numa coluna separada
- Corrigido em 8 arquivos, adicionando `min-w-0` no lado do texto e `shrink-0` no lado fixo: [src/app/(dashboard)/page.tsx](<src/app/(dashboard)/page.tsx>) (transações recentes), [transacoes/TransactionRow.tsx](<src/app/(dashboard)/transacoes/TransactionRow.tsx>), [contas/AccountRow.tsx](<src/app/(dashboard)/contas/AccountRow.tsx>), [categorias/CategoryRow.tsx](<src/app/(dashboard)/categorias/CategoryRow.tsx>), [metas/GoalRow.tsx](<src/app/(dashboard)/metas/GoalRow.tsx>) (título da meta e cada linha do extrato), [usuarios/UserRow.tsx](<src/app/(dashboard)/usuarios/UserRow.tsx>) (nome/email truncados com `truncate`, já que é uma lista compacta), [orcamentos/page.tsx](<src/app/(dashboard)/orcamentos/page.tsx>) (nome da categoria e o banner de "estourou o orçamento em"), [components/MobileNav.tsx](src/components/MobileNav.tsx) (nome do usuário na gaveta mobile, agora truncado) e [layout.tsx](<src/app/(dashboard)/layout.tsx>) (nome do usuário no header desktop)
- De brinde: a bolinha de cor ao lado do nome da categoria (Categorias e Orçamento) estava alinhada ao centro vertical do bloco todo, então quando o nome quebrava em 2 linhas a bolinha ficava grudada na segunda linha em vez da primeira — trocado `items-center` por `items-start` com uma margem no topo da bolinha, pra alinhar com a primeira linha do texto
- Também corrigido: o placeholder "Sem orçamento" do campo de valor em `/orcamentos` cortava no meio ("Sem orçament") por o input ser estreito demais (`w-32` → `w-36`)
- **Testado de ponta a ponta de novo** (mesmo processo da vez anterior: Postgres local temporário + `prisma migrate deploy` + seed via rota temporária `/api/dev-seed`, removida ao final): usei nomes de categoria/conta/usuário propositalmente longos pra forçar o bug, tirei screenshots em 375px (iPhone SE) antes e depois da correção e também comparei as coordenadas (bounding boxes) do texto e do valor via Playwright pra confirmar que pararam de se tocar/sobrepor. Como este ambiente não tem o WebKit do Playwright instalado (só Chromium), a correção foi validada via Chromium + revisão de código, não no Safari real — o padrão do bug (flexbox sem `min-w-0`) é bem conhecido e a correção é a mesma independente do motor, mas vale o usuário conferir num iPhone de verdade quando puder
- Validado com `tsc --noEmit`, `eslint` e `next build`
